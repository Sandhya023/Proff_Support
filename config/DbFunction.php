...
function register($cshort,$cfull,$fname,$lname,$gender,$mobno,$email,$country,$state,$city,$Quiz, $InSem1, $InSem2, $EndSem, $Lab, $Labattendance, $Submission, $Viva){
 			          
 			        $db = Database::getInstance();
		           	$mysqli = $db->getConnection();
		           	
		           //	echo $session;exit;
   $query = "INSERT INTO `registration` (`course`, `subject`, `fname`, `lname`, `gender`,  `mobno`, `emailid`, `country`, `state`, `dist`, `Quiz`, `InSem1`, `InSem2`, `EndSem`, `Lab`, `Labattendance`, `Submission`, `Viva` , regno) 
                   VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
			        $reg=rand();
			        $stmt= $mysqli->prepare($query);
			        if(false===$stmt){
			...
			$stmt->bind_param('sssssssssssssssssss',
		         	$cshort,$cfull,$fname,$lname,$gender,$mobno,
					$email,$country,$state,$city,$Quiz, $InSem1,$InSem2, $EndSem, $Lab, $Labattendance, $Submission, $Viva ,$reg);
			$stmt->execute();
...
function edit_std($cshort,$cfull,$fname,$lname,$gender,$mobno,$email,$country,$state,$city,$Quiz, $InSem1, $InSem2, $EndSem, $Lab, $Labattendance, $Submission, $Viva,$id){
  // echo $id;exit;
    $db = Database::getInstance();
	$mysqli = $db->getConnection();
	$query = "update registration set course=?,subject=?,fname=?,lname=?,gender=?,mobno=?,emailid=?,country=?,state=?,dist=?,Quiz=?,InSem1=?,InSem2=?,EndSem=?, Lab=?,Labattendance=?,Submission=?,Viva=? where id=?" ;
    //echo $query;
	$stmt= $mysqli->prepare($query);
	if(false===$stmt){
	...
	$rc=$stmt->bind_param('ssssssssssssssssssssi',$cshort,$cfull,$fname,$lname,$gender,              $mobno,$email,$country,$state,$city,$Quiz, $InSem1, $InSem2, $EndSem,$Lab,$Labattendance,$Submission,$Viva,$id);
